...
def compute_log_frequency_for_lang(lang_word_list_path, base=2, sqrt=False, normalize=False):
    ...
    for word, count in cnt.most_common():
        ...
        log_freq = -math.log2(rel_freq)
        if base == 10:
            log_freq = -math.log10(rel_freq)
        if sqrt:
            log_freq = math.sqrt(1 / log_freq)
        word_rel_log_freq[word] = log_freq
        # print(-math.log2(rel_freq))
        # print(w[0])
    # print(total)
...
def plot_annotate_top_n(source_word, target_word, best, cos_sim_list, word_list, marker_size=1, annotate_top_n=10, show_top_n=10, tokenizer=None, parameters=[]): # title, words
    ...
    title = source_word + ">" + target_word + " top_" + str(show_top_n)
    for arg in parameters:
        title += " " + arg + " "
    plt.title(title)
    for index in best[:annotate_top_n - 1]:
        # change annotate to also work for sorted version
        if tokenizer is not None:
            text = "(" + tokenizer.convert_ids_to_tokens(index.item()) + ", " + str(np.around(cos_sim_dict[index], decimals=5)) + ")"
            plt.annotate(text, xy=(index, cos_sim_dict[index]), fontsize=7)
        else:
            plt.annotate(index, xy=(index, cos_sim_dict[index]))
    img_path = '../data/plots/' + title + '.svg'
    fig = plt.gcf()
...
#
# def create_img_path(title, i):
#     img_title =
#     # img_title.replace(" ", "-")
#     print(img_title)
#     img_path = Path(img_title)
#     if img_path.is_file():
#         create_img_path(title+str(i+1), i+1)
#     else:
#         return img_title
...
def main():
    ...
    show = [10, 50, 100, 1000, 10000, None]
    ...
    word_log_freq_dict = compute_log_frequency_for_lang("/Users/zhugegao/PycharmProjects/BAthesis_ZG/data/en/en_50k.txt", base=2, sqrt=False,
        normalize=False)
    word_list = list(word_log_freq_dict.keys())
    word_list = word_list[:highlight-1]
    ...
    with open("../data/word_lists/analogy_word_pairs.csv") as words:
        ...
        for row in reader:
            ...
            b1_baseline_rank, _ = sort_vocab_with_log_freq(b2_emb, output_embeddings, tokenizer_mlm, word_log_freq_dict,
                                                        binary_freq=binary, norm_vec=True, square=square, norm_cos_sim_list=norm_cos,
                                                        mean=mean)
            ...
            b1_id_ranks, b1_cos_list = sort_vocab_with_log_freq(new_b1_emb, output_embeddings, tokenizer_mlm, word_log_freq_dict,
                                                   binary_freq=binary, norm_vec=True, square=square, norm_cos_sim_list=norm_cos,
                                                   mean=mean)
            parameters_b1 = parameters.copy()
            parameters_b1.append("(" + a2 + " > "+ a1 + ")")
            for s in show:
                plot_annotate_top_n(b2, b1, b1_id_ranks, b1_cos_list, word_list=word_list, annotate_top_n=annotate, show_top_n=s,
            ...
            b2_baseline_rank, _ = sort_vocab_with_log_freq(b1_emb, output_embeddings, tokenizer_mlm, word_log_freq_dict,
                                                        binary_freq=binary, norm_vec=True, square=square, norm_cos_sim_list=norm_cos,
                                                        mean=mean)
            ...
            b2_id_ranks, b2_cos_list = sort_vocab_with_log_freq(new_b2_emb, output_embeddings, tokenizer_mlm, word_log_freq_dict,
                                                   binary_freq=binary, norm_vec=True, square=square, norm_cos_sim_list=norm_cos,
                                                   mean=mean)
            for s in show:
                plot_annotate_top_n(b1, b2, b2_id_ranks, b2_cos_list, word_list=word_list, annotate_top_n=annotate, show_top_n=s,
    ...
    categories_word_list = ["instrument", "instrument_inv", "crime", "crime_inv", "growth", "growth_inv", "profession", "profession_inv"]
    category_output = ""
    for category in categories_word_list:
        ...
        for i, l in enumerate(leave_one_out_list):
            ...
            print(w, t)
            baseline_ranks, _ = sort_vocab_with_log_freq(w_emb,  output_embeddings, tokenizer_mlm, word_log_freq_dict, binary_freq=binary, norm_vec=True, square=square, norm_cos_sim_list=norm_cos, mean=mean)
            ...
            t_id_ranks, t_cos_list = sort_vocab_with_log_freq(t_emb_new, output_embeddings, tokenizer_mlm, word_log_freq_dict, binary_freq=binary, norm_vec=True, square=square, norm_cos_sim_list=norm_cos, mean=mean)
            parameters_t = parameters.copy()
            parameters_t.append("category="+category)
            for s in show:
                plot_annotate_top_n(w, t, t_id_ranks, t_cos_list, word_list=word_list, annotate_top_n=annotate, show_top_n=s, tokenizer=tokenizer_mlm)
            target = w + ">" + t
        ...
        category_output += category + " improve from baseline: " + str(t_win)+ " out of " + str(len(leave_one_out_list)) + "\n"
